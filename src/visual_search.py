...
def main():
    ...
    parser.add_argument("--query-index", type=int, default=None, help="Index in test set")
    ...
    print("Extracting features for search...")
    feats, views, seq_types, labels = model_obj.transform('test', batch_size=1)
    feats_torch = torch.from_numpy(feats)
    
    # Pick a query
    q_idx = args.query_index if args.query_index is not None else np.random.randint(len(feats))
    q_feat = feats_torch[q_idx].unsqueeze(0)
    q_label = labels[q_idx]
    q_info = test_df.iloc[q_idx]
    
    # Similarity (Cosine if normalized, but let's use dot product for now or Euclidean)
    # Model.transform doesn't always normalize, so let's normalize manually for search
    q_feat_norm = torch.nn.functional.normalize(q_feat, p=2, dim=1)
    feats_norm = torch.nn.functional.normalize(feats_torch, p=2, dim=1)
    
    dist = torch.mm(q_feat_norm, feats_norm.t()).squeeze(0)
    ...
    plt.suptitle(f"Visual Search - {cfg['save_name']}")
    plt.tight_layout()
    
    output_dir = Path("outputs") / cfg['save_name'] / "visual_search"
    output_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(output_dir / f"search_idx_{q_idx}.png")
